...
- (void)authorizationController:(ASAuthorizationController *)controller didCompleteWithAuthorization:(ASAuthorization *)authorization  API_AVAILABLE(ios(13.0)){
    if ([authorization.credential isKindOfClass:[ASAuthorizationAppleIDCredential class]]) {
        ...
        LogDebug(@"user: %@", credential.user);
        LogDebug(@"fullName: %@%@%@", credential.fullName.familyName?:@"", credential.fullName.middleName?:@"", credential.fullName.givenName?:@"");
        LogDebug(@"email: %@", credential.email);
        LogDebug(@"authorizationCode: %@", [[NSString alloc] initWithData:credential.authorizationCode encoding:NSUTF8StringEncoding]);
        LogDebug(@"Identify: %@", [[NSString alloc] initWithData:credential.identityToken encoding:NSUTF8StringEncoding]);
    }
}
